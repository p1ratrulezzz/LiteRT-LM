...
namespace {
    ...
    public:
        StreamingObserver(int sock, std::atomic<bool> &cancelled) : sock_(sock), cancelled_(&cancelled),
                                                                    tokens_sent_(0), total_bytes_sent_(0),
                                                                    repetition_count_(0), max_repetitions_(3),
                                                                    stop_sending_(false), closed_(false) {
            ...
            for (size_t len = 15; len <= 80 && len <= recent_chars_.size() / 2; len += 5) {
                ...
                if (last_sequence == prev_sequence && !last_sequence.empty()) {
                    ...
                    if (has_meaningful_content) {
                        ABSL_LOG(WARNING) << "Found repeating phrase of length " << len << ": '" << last_sequence.
substr(0, 30) << "...'";
                        return true;
                    }
                }
    ...
    absl::Status MainHelper(int argc, char **argv) {
        ...
        while (true) {
            ...
            while ((bytes_read = recv(client_sock, buffer, sizeof(buffer) - 1, 0)) > 0) {
                ...
                ABSL_LOG(INFO) << "Received chunk #" << recv_count << " (" << bytes_read << " bytes): " << std::string(
                                  buffer, bytes_read);
            ...
            ABSL_LOG(INFO) << "Finished reading JSON. Total chunks: " << recv_count << ", Total bytes: " <<
 total_bytes_received;
            ...
            ABSL_LOG(INFO) << "Final processed JSON (length: " << input_prompt.length() << "): " << input_prompt.
substr(0, 500) << (input_prompt.length() > 500 ? "..." : "");
